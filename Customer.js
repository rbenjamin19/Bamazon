...
//function to print all items to the console
function printItems(cb){
  //cli-table
  var table = new Table({
    head: ['ID Number', 'Product', 'Department', 'Price', 'Quantity Available']
  });
  //obtain all rows from the Products table
  connection.query('SELECT * FROM Products', function(err, res){
    ...
    //prompt the user to add items to cart
    cb();
    });
  }
...
function userSelectsItem(){
  var items = [];
  //obtain all product names from the Products table
...
function checkout(){
  ...
  if (shoppingCart.length != 0) {
    ...
    console.log('---------------------------------------------');
    console.log('Here is your shopping cart. Are you all ready to checkout?');
    ...
    inquirer.prompt([
      ...
      name: 'choice',
      type: 'list',
      message: 'Oh no! Your cart is empty. Would you like to keep shopping or leave?',
      choices: ['Keep Shopping', 'Leave']
      }
    ]).then(function(user){
        //if keep shopping is selected print the items and prompt the user to select items again
...
function editItem(itemsToEdit){
  ...
  if (itemsToEdit.length != 0) {
    ...
    inquirer.prompt([
      ...
      choices: ['Remove From My Cart', 'Change Quantity']
      }
    ]).then(function(user){
